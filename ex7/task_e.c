...
void my_func(void *args) {
	...
	if(a==1){
		...
		rt_task_set_priority(&my_task[a-1],50);
		rt_mutex_acquire(&my_mutex_a,TM_INFINITE);
		rt_printf("Task %d - Got a mutex A\n", a);
		busy_wait_us(300000);
		rt_mutex_acquire(&my_mutex_b,TM_INFINITE);
		rt_printf("Task %d - Got a mutex B\n", a);
		busy_wait_us(300000);
		rt_mutex_release(&my_mutex_b);
		rt_printf("Task %d - Released a mutex B\n", a);
		rt_mutex_release(&my_mutex_a);
		rt_printf("Task %d - Released a mutex A\n", a);
		rt_task_set_priority(&my_task[a-1],40);
		busy_wait_us(100000);
	} else if(a==2){		
...
int main(){
	...
	int my_task_prio[NUM_TASK] = {40,50};
	int my_task_args[NUM_TASK] = {1,2};

	rt_sem_create(&my_sem, NULL, SEM_INIT, SEM_MODE);
	rt_mutex_create(&my_mutex_a, NULL);
	rt_mutex_create(&my_mutex_b, NULL);
	...
	rt_mutex_delete(&my_mutex_a);
	rt_mutex_delete(&my_mutex_b);
